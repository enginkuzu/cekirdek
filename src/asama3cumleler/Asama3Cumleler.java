...
import asama2sozcukler.Sozcuk_00Yok;
	...
	private static boolean satırıİşle(Sozcuk_06Degisken hedefDeğişken, Sozcuk[] cümle, int minIndex, int maxIndex) {
		...
		while (minIndex != maxIndex) {
			...
			for (int i = index; i <= maxIndex; i++) {
				if (cümle[i].tip != SÖZCÜK.TİP_00YOK) {
					if (sozcuk0.tip == SÖZCÜK.TİP_00YOK) {
						index0 = i;
						sozcuk0 = cümle[i];
					} else if (sozcuk1.tip == SÖZCÜK.TİP_00YOK) {
						index1 = i;
						sozcuk1 = cümle[i];
			...
			if (sozcuk2.tip == SÖZCÜK.TİP_14AÇ_PARANTEZ) {
				...
				// -sayı
				// -----------------
				Sozcuk_06Degisken kaynakDeğişken2 = değişkeneDönüştür(sozcuk1);
				if (kaynakDeğişken2 == null)
					return false;
				//
				String operatör1 = ((Sozcuk_02Operator) sozcuk0).operatör;
				String anahtar1 = Degiskenler.ID_none + operatör1 + kaynakDeğişken2.değişkenTipiId;
				if (!operatörFonksiyonMap.containsKey(anahtar1)) {
					hata04_TanımsızOperatör(
							operatör1 + " " + Degiskenler.TİP_MAP_ID_STR.get(kaynakDeğişken2.değişkenTipiId));
					return false;
				}
				Fonksiyon_01OperatorFonksiyon fonksiyon = operatörFonksiyonMap.get(anahtar1);
				//
				...
				Cumle_02GeciciDegiskenYeni yeniCümle1 = new Cumle_02GeciciDegiskenYeni(yeniDeğişkenNo,
						yeniDeğişkenTipId);
				aktifFonksiyon.cümleler.add(yeniCümle1);
				Cumle_04OperatorIslemi yeniCümle2 = new Cumle_04OperatorIslemi(yeniDeğişkenNo, operatör1, 0,
						kaynakDeğişken2.değişkenNo);
				aktifFonksiyon.cümleler.add(yeniCümle2);
				//
				cümle[index0] = new Sozcuk_00Yok();
				cümle[index1] = yeniDeğişken;
				if (minIndex == index)
					minIndex = index1;
				index = index1;
				//
				...
				if (kaynakDeğişken2 == null)
					return false;
				//
				String operatör1 = ((Sozcuk_02Operator) sozcuk1).operatör;
				String anahtar1 = kaynakDeğişken1.değişkenTipiId + operatör1 + kaynakDeğişken2.değişkenTipiId;
				if (!operatörFonksiyonMap.containsKey(anahtar1)) {
					hata04_TanımsızOperatör(Degiskenler.TİP_MAP_ID_STR.get(kaynakDeğişken1.değişkenTipiId) + " "
							+ operatör1 + " " + Degiskenler.TİP_MAP_ID_STR.get(kaynakDeğişken2.değişkenTipiId));
					return false;
				}
				Fonksiyon_01OperatorFonksiyon fonksiyon1 = operatörFonksiyonMap.get(anahtar1);
				//
				int index3 = -1;
				Sozcuk sozcuk3 = new Sozcuk_00Yok();
				for (int i = index2 + 1; i <= maxIndex; i++) {
					if (cümle[i].tip != SÖZCÜK.TİP_00YOK) {
						index3 = i;
						sozcuk3 = cümle[i];
						break;
				...
				if (sozcuk3.tip == SÖZCÜK.TİP_08NOKTA) {
					index = index2;
					continue;
				}
				//
				if (sozcuk3.tip == SÖZCÜK.TİP_02OPERATÖR) {
					...
					if (sozcuk4.tip == SÖZCÜK.TİP_14AÇ_PARANTEZ) {
						...
						if (kaynakDeğişken3 == null)
							return false;
						//
						String operatör2 = ((Sozcuk_02Operator) sozcuk3).operatör;
						String anahtar2 = kaynakDeğişken2.değişkenTipiId + operatör2 + kaynakDeğişken3.değişkenTipiId;
						if (!operatörFonksiyonMap.containsKey(anahtar2)) {
							hata04_TanımsızOperatör(Degiskenler.TİP_MAP_ID_STR.get(kaynakDeğişken2.değişkenTipiId) + " "
									+ operatör2 + " " + Degiskenler.TİP_MAP_ID_STR.get(kaynakDeğişken3.değişkenTipiId));
							return false;
						}
						Fonksiyon_01OperatorFonksiyon fonksiyon2 = operatörFonksiyonMap.get(anahtar2);
						if (fonksiyon2.öncelik > fonksiyon1.öncelik) {
							index = index2;
							continue;
						}
					}
				...
				//
				int yeniDeğişkenNo = --aktifFonksiyon.geçiciDegiskenNo;
				int yeniDeğişkenTipId = fonksiyon1.sonuçTipId;
				Sozcuk_06Degisken yeniDeğişken = new Sozcuk_06Degisken(yeniDeğişkenNo, null, yeniDeğişkenTipId);
				aktifFonksiyon.değişkenNoMap.put(yeniDeğişkenNo, yeniDeğişken);
				Cumle_02GeciciDegiskenYeni yeniCümle1 = new Cumle_02GeciciDegiskenYeni(yeniDeğişkenNo,
						yeniDeğişkenTipId);
				aktifFonksiyon.cümleler.add(yeniCümle1);
				Cumle_04OperatorIslemi yeniCümle2 = new Cumle_04OperatorIslemi(yeniDeğişkenNo, operatör1,
	...
	public static Object işle(ArrayList<Sozcuk[]> tümCumleler, String dosyaAdı) {
		...
		for (Sozcuk[] cümle : tümCumleler) {
			if (cümle[0].tip == SÖZCÜK.TİP_13ÖZELLİK && cümle[cümle.length - 1].tip == SÖZCÜK.TİP_06SATIR_SONU) {
				...
				if (cümle[3].tip == SÖZCÜK.TİP_02OPERATÖR) {
					//
					String değişken2Tip = ((Sozcuk_01Isim) cümle[7]).isim;
					String sonucTip = ((Sozcuk_01Isim) cümle[11]).isim;
					if (!Degiskenler.TİP_MAP_STR_ID.containsKey(değişken2Tip)) {
					...
					operatörFonksiyon.değişken1İsim = null;
					operatörFonksiyon.değişken1TipId = Degiskenler.ID_none;
					operatörFonksiyon.operatör = ((Sozcuk_02Operator) cümle[3]).operatör;
					operatörFonksiyon.değişken2İsim = ((Sozcuk_01Isim) cümle[5]).isim;
					operatörFonksiyon.değişken2TipId = Degiskenler.TİP_MAP_STR_ID.get(değişken2Tip);
					operatörFonksiyon.sonuçİsim = ((Sozcuk_01Isim) cümle[9]).isim;
					...
					//
				} else if (cümle[7].tip == SÖZCÜK.TİP_02OPERATÖR) {
					//
				...
				// 123.printhn;
				//
				satırıİşle(null, cümle, 0, cümle.length - 2);
				//
			} else {
				// Bilinmeyen Cumle
